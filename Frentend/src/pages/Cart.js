...
import { Col, Container, Row } from 'react-bootstrap';
import {
    Button, Card, Image, notification, Modal, Spin, Empty, Divider, Typography, Badge
} from 'antd';
import Meta from 'antd/es/card/Meta';
import {
    PlusOutlined, MinusOutlined, DeleteOutlined, LoadingOutlined, ShoppingCartOutlined
} from '@ant-design/icons';
import axios from 'axios';
import { useDispatch, useSelector } from 'react-redux';
import { removeFromCart } from '../Redux/Fetures/CartSlice';
        ...
        <Container className="py-4">
            ...
            {loading ? (
                ...
                <>
                    <Title level={2} className="text-center mb-4">
                        <ShoppingCartOutlined /> Your Shopping Cart
                        <Badge count={calculateTotalItems()} style={{ backgroundColor: '#52c41a', marginLeft: 10 }} />
                    </Title>
                    ...
                    <Row gap={4}>
                                                    ...
                                                    <div className="mt-2">
                                                        <div className="d-flex justify-content-between align-items-center">
                                                            ...
                                                            <div className="d-flex align-items-center">
                                                                ...
                                                                <Text strong className="mx-3">{obj.quantity}</Text>
                                                                <Button
                                                                    disabled={buttonLoading[obj._id]}
                                                                    icon={buttonLoading[obj._id] ? <Spin indicator={<LoadingOutlined />} /> : <PlusOutlined />}
                                                                    type="primary"
                        ...
                        <Col xs={24} lg={4} ms={4} className="position-sticky" style={{ top: '20px', zIndex: 10 }}>
                            <Card title="Order Summary" className="shadow-sm">
                                <Row justify="space-between" className="mb-2">
                                    <Col><Text>Subtotal ({calculateTotalItems()} items)</Text></Col>
                                    <Col><Text strong>₹{totalBill}</Text></Col>
                ...
                <div className="text-center mt-5">
                    ...
                    <PaymentMethod price={totalBill} onSuccess={handlePaymentSuccess} />
                    {/* <PaymentPage amount={totalBill} /> */}
                </div>
            ) : (
                <div className="text-center mt-5">
                    <Title level={3} type="success">Order Successful!</Title>
