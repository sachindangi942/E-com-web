...
const { add_product_val, reset_password_val, } = require("../validation/validation");
...
const change_password_controler = async (req, res) => {
    try {
        const { Email } = getToken_data({ headers: req.headers });
        const { oldPassword, newPassword } = req.body;
        const User = await userSchema.findOne({ Email });
        if (!User) return res.status(404).send("Invalid user.");
        const isMatch_password = bcrypt.compareSync(oldPassword, User.password);
        if (!isMatch_password) return res.status(401).send("Invalid old password.");
